...
} from './filter-modal/filter-modal.component';
...
export class StockScreenerComponent implements OnInit, OnDestroy {
  ...
  @ViewChild(FilterModalComponent) filterModal: FilterModalComponent | null =
    null;

  ngOnInit(): void {
    this.tickerSubscription = this.binanceService
    ...
    return this.data.filter(
      (item) =>
        ...
        (filters.minPriceChange == null ||
          item.priceChangePercent >= filters.minPriceChange) &&
        (filters.maxPriceChange == null ||
          item.priceChangePercent <= filters.maxPriceChange) &&
  ...
  protected updateRawData(rawData: Ticker[]) {
    ...
    const filteredData = rawData.filter((crypto) =>
  ...
  protected applyDataChanges(newData: Ticker[]): void {
    ...
    for (const ticker of newData) {
      const existingTicketIndex = this.rawData.findIndex(
